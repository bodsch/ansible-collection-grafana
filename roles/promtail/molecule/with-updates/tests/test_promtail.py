...
@pytest.mark.parametrize("files", [
    ...
    "/lib/systemd/system/promtail.service",
    "/usr/bin/promtail"
])
...
def test_version(host, get_vars):
    ...
    version_dir = f"/usr/local/bin/promtail/{version}"
    version_file = f"{version_dir}/promtail"
    current_link = "/usr/bin/promtail"
    ...
    f = host.file(version_file)
    assert f.is_file

    link  = host.file(current_link)
    assert link.is_symlink
    assert link.linked_to == version_file
